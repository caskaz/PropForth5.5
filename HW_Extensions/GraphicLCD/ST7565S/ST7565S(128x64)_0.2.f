...
\ Set ports for [D0-D7] to output
\ ( -- )
: set_DataOut dira COG@ mask and dira COG! ;
\ Get [D0-D7]
\ ( -- )
: Get_data ina COG@ _data0 rshift hFF and ;

\ Read status and DDRAM from ST7565
\ ( n1 n2 -- n3 ) n1:bytes(>1)[not need when status] n2:status/data  n3:reading data
: rd_st7565
_cs pinlo                     \ Set CS to lo
...
_cs pinhi                     \ Set cs to hi
\ Back ports for [D0-D7] to output
mask invert dira COG@ or dira COG!
;
...
: rd_stat
status rd_st7565  
dup h80 and if ." Busy" else ." Not Busy" then cr
dup h40 and if ." ADC:Normal" else ." ADC:Reverse" then cr
dup h20 and if ." Display OFF" else ." Display ON" then cr
h20 and if ." Reset in progress" else ." Operating state" then cr
cr
;

\ DDRAM position 
\ ( n1 n2 -- ) n1:x[0-127] n2:y[0-7]
: posDDRAM
\ y-position
...
\ Display DDRAM(n1 bytes)
\ Position must be defined by posDDRAM or pos before executing this word
\ ( n1 -- ) n1:byte(>1)
: rd_DDRAM data hex rd_st7565 decimal ;
...
\ 8x8Character Position in DDRAM
...
\ Selected font(8x8 or prop) by variable'font_type'
...
dup h80000000 and if negate h2D print then             \ Check if minus and print "-"  
d10 0 do 
     dup tmp L@ >=                                     \ Check if n is bigger than tmp
     if 
          tmp L@ u/mod h30 + print 1 result W!         \ Print number-char
     else 
          result W@ tmp L@ 1 = or 
          if h30 print then                            \ Print "0"
     then
...
: demo4
init_LCD
\ Display number(8x8font) on column1 of line1 
1 font_type W!           \ 8x8font      
0 x W! 0 y W!
1234567 lcd_dec
\ Display number(8x8font) on column6 of line2
5 x W! 1 y W!
-91235 lcd_dec
\ Display string'PropForth5.5'(8x8font) on column1 of line3
0 x W! 2 y W!
c" PropForth5.5" lcd_string

0 font_type W!           \ propfont
\ Display number(propfont) on column1 of line4(position for 8x8font)
0 x W! 4 y W!
-5216 lcd_dec
