...
2015/10/06 15:33:39
   ...
   RTC DS1337     Propeller
      scl    ----  P28   
      sda    ----  P29 
   SQW/INT   ----  Pull-up resistor(10kohm) --- 3.3V
...
\ SQW_rate
0 wconstant 1Hz     \ 1Hz
1 wconstant 1kHz    \ 1.024kHz
2 wconstant 4kHz    \ 4.096kHz
3 wconstant 8kHz    \ 8.192kHz
...
if 
     ...
     hFE and                            \ ( data*hFE )       
     \ Clear A1M4
     alm1_DY/DT dup DS3231 i2c_rd       \ ( n1*hFE register data )
     h7F and swap DS3231 i2c_wr         \ ( n1*hFE )   
else 
     \ clear bit1
     hFD and                            \ ( data*hFD )                
     \ Clear A2M4
     alm2_DY/DT dup DS3231 i2c_rd       \ ( data*hFDE register data )
     h7F and swap DS3231 i2c_wr         \ ( n1*hFE ) 
     status DS3231 i2c_wr 
then
chk_INT
;
...
dup alarm2 = if 1+ else 2+ then              \ Get Hour regiter address
                                             \ ( n1 n2 )
2dup                                         \ ( n1 n2 n1 n2 )
DS3231 i2c_rd                                \ Read Hour register  ( n1 n2 n1 data )
12Hour and 6 rshift <>                       \ ( n1 n2 t/f )
if
                                             \ ( n1 n2 )
     dup DS3231 i2c_rd                       \ ( n1 n2 data )
     dup 12Hour and 0 =                                             
     if
...
: disp_DY/DT
DS3231 i2c_rd                                          \ Get Day/Date for alarm1 or alarm2
                                                       \ ( n1 data )
...
\ n7 - yesr		(2000 - 2099)   
: rd_current
7 0 do current i + DS3231 i2c_rd bcd> loop                 
cr   ." CurrentTime" cr
." Year: " d2000 + . cr
...
\ n5 - minute		(00 - 59)
: set_current 
0 status DS3231 i2c_wr        \ Clear OSF
>bcd >r                       \ minute
>bcd >r                       \ hour
>r                            \ day-of-week
>bcd >r                       \ day
...
d2000 - >bcd                  \ year
r> r> r> r> r> 0     
                    \ 7 0 do current i + DS3231 i2c_wr err? loop
7 current DS3231 i2c_wr_multi
rd_current 
0 alarm1 set_12/24 0 alarm2 set_12/24        \ Set alarm1/alarm2 to 24Hour mode

\ Clear A1M1 
alarm1 DS3231 i2c_rd h7F and alarm1 DS3231 i2c_wr 
\ Clear A1M3 
alm1_Hour DS3231 i2c_rd h7F and alm1_Hour DS3231 i2c_wr 
\ Clear A2M3
alm2_Hour DS3231 i2c_rd h7F and alm2_Hour DS3231 i2c_wr 
\ Clear A2M4 
alm2_DY/DT DS3231 i2c_rd h7F and alm2_DY/DT DS3231 i2c_wr 
;

\ =========================================================================== 
\ Alarm1 time
\ =========================================================================== 
\ Get alarm1 time from DS3231
\ ( -- n1 n2 n3 n4 )
\ n1 - second		(00 - 59)
\ n2 - minute		(00 - 59)
\ n3 - hour		(00 - 23)
...
\      date  (1-31)
: rd_alm1
4 0 do alarm1 i + DS3231 i2c_rd bcd> loop            
...
\ Set alarm1-time to DS3231   (24Hour mode)
\ Set second to 0
\ ( n1 n2 n3 -- ) 
...
r> r> r> 0
2 alarm1 DS3231 i2c_wr_multi                 \ Save second and minute to register                                           
\ Hour   
alm1_Hour conv_hour                                                     
alm1_Hour DS3231 i2c_wr                      \ Save Hour to register          
alm1_DY/DT DS3231 i2c_rd 
DY/DT h80 or and or 
alm1_DY/DT DS3231 i2c_wr                    \ Save bit[A1M4] + bit[Day or Date] + InputValue to register
...
\      date
: rd_alm2
3 0 do alarm2 i + DS3231 i2c_rd bcd> loop
...
\ Set alarm2-time to DS3231   (24Hour mode)
\ ( n1 n2 n3 -- ) 
\ n1 - date		(01 - 31)         bit[DY/DT]=0
\ n1 - day-of-week  (Mon:1 Tue:2 Wed:3 Thur:4 Fri:5 Sat:6 San:7)   bit[DY/DT]=1
\ n2 - hour		(00 - 23)
...
>bcd >r                       \ day / day-of-week
r> r> r>      
alarm2 DS3231 i2c_wr          \ Save minute to register
\ Hour
alm2_Hour conv_hour
alm2_Hour DS3231 i2c_wr       \ Save Hour to register          
 
alm2_DY/DT DS3231 i2c_rd 
DY/DT h80 or and or 
alm2_DY/DT DS3231 i2c_wr      \ Save bit[A2M4] + bit[Day or Date] + InputValue to register
...
\ =========================================================================== 
\ Manipulating special-purpose registers for osc
\ =========================================================================== 
\ Enable/Disable 32.768kHz from 32kHz-pin(1)
\ ( n1 -- )  n1:0(disable)/1(enable)
: 32kHz_out
hF DS3231 i2c_rd 
2dup                     \ ( n1 data n1 data )
8 and 3 rshift <>        \ ( n1 data t/f )
if
     swap                \ ( data n1 )
     if
          \ Output 32.768kHz
          8 or hF DS3231 i2c_wr 
     else
          \ Stop 32.768kHz
          hF7 and hF DS3231 i2c_wr 
     then
else
     swap                \ ( data n1 )
     if
...
\ Set SQW
\ ( n1 --)  n1:1Hz or 1kHz(1.042kHz) or 4kHz(4.096kHz) or 8kHz(8.192kHz) (default:working as INT)
: set_SQW 
control DS3231 i2c_rd 
hE7 and swap 3 lshift or 
control DS3231 i2c_wr 
;

\ Enable\Disable osc when battery backup
\ When DS3231 is powered by Vcc, osc is always on regardless of the status of the EOSC bit
\ ( n1 -- ) n1:0(disable)/1(enable)
: EOSC
control DS3231 i2c_rd swap              \ ( data n1 )
if
     \ enable osc when battery backup    ( data )
     h7F and 
else
     \ disable osc when battery backup   ( data )
     h80 or 
then
control DS3231 i2c_wr 
;
...
\ ( -- )  n1:0(stop)/1(start)
: osc_state
status DS3231 i2c_rd 
h80 and
if
     ." Oscillator is stopped or was stopped for some period." cr
     status DS3231 i2c_rd h7F and status DS3231 i2c_wr 
     ." Oscillator start"
else
     ." Oscillator operate"
then
...
\ ( -- )
: getTemp
control DS3231 i2c_rd         \ Read control register 
h20 or 
control DS3231 i2c_wr         \ Write setting CONV-bit to control register 
h11 DS3231 i2c_rd             
. h2E emit
h12 DS3231 i2c_rd             
h40 over =
if h32 emit h35 emit
else h80 over =
if h35 emit
else hC0 over =
if h37 emit h35 emit
else h30 emit
thens
drop 
." degree" cr
...
begin
     ...
     begin status DS3231 i2c_rd 4 and 0= until
     \ Get temperature  
     getTemp cr
     fkey? swap drop
until
...
if
     ...
     if
          ...
          h7F and                            \ ( data )
     then
     alm1_DY/DT                              \ ( data register )
else
     \ alarm2
     if
          \ ignore DY/DT (Set bit[A2M4]) 
          alm2_DY/DT DS3231 i2c_rd           \ ( data )
          h80 or                        \ ( data )
     else
...
DS3231 i2c_rd                                     \ ( register n1 data )
